const apiKey = (window.CONFIG && window.CONFIG.apiKey) || '';
if (!apiKey) {
  console.error('Gemini API key is missing. Please check config.js');
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function callGeminiAPI(docs) {
        const apiUrl = `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-05-20:generateContent?key=${encodeURIComponent(apiKey)}`;

        const systemPrompt = "You are a 'Smart Doc Checker' agent. Your task is to analyze multiple documents, identify contradictions and significant overlaps, and suggest clear resolutions. Provide your output in a valid JSON format according to the provided schema. If there are no issues, return empty arrays for 'contradictions' and 'overlaps'. Do not include any text outside of the JSON object.";

        const userQuery = docs.map((doc, i) => `Document ${i + 1} Name: ${doc.name}\n---\n${doc.content}\n---`).join('\n\n');
        ...
        try {
            ...
            if (!response.ok) {
                ...
                const detail = errorBody?.error?.message || `The server responded with status ${response.status}. Please try again later.`;
            ...
            if (jsonString) {
                ...
                throw new Error("The AI model returned an invalid response. It might be due to the complexity of the documents.");
            }
        } catch (error) {
